...
for i = setdiff(1:length(variables),2:5) %exclude 2:5
    filtered_data.(variables{i}) = NaN_data.(variables{i});
end
...
for j = 1:length(variables)
    for i = 1:size(filtered_data.(variables{j}),1)
        if isdatetime(filtered_data.(variables{j})) == 0
            X = filtered_data.(variables{j})(i,:);
            X(isnan(X)) = interp1(find(~isnan(X)), X(~isnan(X)), find(isnan(X)), 'linear'); %linear interpolation
            interp_data.(variables{j})(i,:) = X;
            clear X 
        else ;
        end
    end
...
for i = 1%:length(median_alt_row1)   
    for j = parameters_of_interest
        [input_pks{i,j},input_locs{i,j}] = findpeaks(interp_data.(variables{j})(row_interest(i),:),interp_data.Time(1,:));%,'MinPeakHeight',max(powerspectrum{i,j},[],'all')-10,'MinPeakProminence',7); 
        [input_pks1{i,j},input_locs1{i,j}] = findpeaks(-interp_data.(variables{j})(row_interest(i),:),interp_data.Time(1,:));%,'MinPeakHeight',max(powerspectrum{i,j},[],'all')-10,'MinPeakProminence',7); 
        ...
        figure()
        plot(interp_data.Time(1,:),interp_data.(variables{j})(row_interest(i),:))
        title(strcat([variables{j},'; Altitude: ',alt_str(i,:),'; ',datestr(NaN_data.Time_datetime(1),'yyyy-mm-dd')]))
        datetick('x',13,'keeplimits')
        xlim([n11 n22])
        hold on
...
for i = 1:length(median_alt_row1)%length(suitable_rows)%length(alt_lim)-1
    for j = parameters_of_interest % 1:length(variables)
        if length(interp_data.(variables{j})) > window_length_seconds/T %meaning the period of interest is larger than 1 hour.
            ...
            N_1 = length(interp_data.(variables{j}));
        end
