...
hold on
semilogy(Time(1,:)',el)
semilogy(Time(1,:)',sys_temp)
semilogy(Time(1,:)',peak_power)
xlim(xLimits);
% ylim(yLimits);
datetick('x',13,'keeplimits')
...
for i = 2:5
    poi_averaged.(variables{i}).Eregion = nanmedian(NaN_data.(variables{i})(find(NaN_data.alt(:,1) > E_lower_lim & NaN_data.alt(:,1) < F_lower_lim),:));
%     poi_averaged.(variables{i}).Eregion2 = nanmedian(NaN_data.(variables{i})(find(NaN_data.alt(:,1) > E_mid_lim & NaN_data.alt(:,1) < F_lower_lim),:));
    poi_averaged.(variables{i}).Fregion = nanmedian(NaN_data.(variables{i})(find(NaN_data.alt(:,1) > F_lower_lim & NaN_data.alt(:,1) < F_high_lim),:));
end
for i = 6:7
...
for i = 2:5 %only apply the high pass filter for ne, te, ti and vi (NOT for alt, Time and Time_datetime)
    for j = regions_of_interest %1:length(str_region)
        filtered_data.(variables{i}).(str_region{j}) = poi_averaged.(variables{i}).(str_region{j}) - movmean(poi_averaged.(variables{i}).(str_region{j}),20);
    end
end
...
for j = 2:5
    for i = regions_of_interest %1:length(str_region)
            ...
            Y = poi_averaged.(variables{j}).(str_region{i});
            Y(isnan(Y)) = interp1(find(~isnan(Y)), Y(~isnan(Y)), find(isnan(Y)), 'linear'); %linear interpolation
            unfiltered_data.(variables{j}).(str_region{i}) = Y;            
            clear X Y
    end
end
...
%         modulation.(variables{j}).(str_region{i}) = (interp_data.(variables{j}).(str_region{i})-movmean(interp_data.(variables{j}).(str_region{i}),20))./movmean(interp_data.(variables{j}).(str_region{i}),20);
        modulation.(variables{j}).(str_region{i}) = (unfiltered_data.(variables{j}).(str_region{i})-movmean(unfiltered_data.(variables{j}).(str_region{i}),20))./movmean(unfiltered_data.(variables{j}).(str_region{i}),20);
...
for i = regions_of_interest %1:length(str_region)%length(suitable_rows)%length(alt_lim)-1
    for j = parameters_of_interest % 1:length(variables)
        for k = 5:size(powerspectrum{i,j},2)-5
            [pks{i,j,k},locs{i,j,k}] = findpeaks(powerspectrum{i,j}(:,k), F_Fregion{i,j}(idx_freq_pc5),'MinPeakProminence',15); %'MinPeakHeight',max(powerspectrum{i,j},[],'all')-10, %IDEA: ONLY USE THE MAXIMUM POWER WITHIN A 3 HOUR INTERVAL AROUND THE POINT K
        end
...
% color_region = jet(length(str_region));
% for j = parameters_of_interest
%     figure()
%     hold on
%     ylim([0 1000/120])
...
%     box on
%     for i = regions_of_interest %1:length(str_region)
%         plot(n11,0,'*','color',color_region(i,:))
%     end
%     legend(str_region)
%     for i = regions_of_interest %1:length(str_region)
%         for k = 5:size(P_Fregion{i,j},2)-5
%             if isempty(locs{i,j,k}) == 0
%                 plot(Time1(k),locs{i,j,k}*1000,'*','color',color_region(i,:),'HandleVisibility','off')
%             else ;
%             end
%         end
%     end
...
% color_parameters1 = jet(length(parameters_of_interest));
% for i = regions_of_interest %1:length(str_region)
%     figure()
%     box on
%     hold on
...
%     xlim([datenum(datetime(n11,'ConvertFrom','datenum')+seconds(window_length_seconds)) datenum(datetime(n22,'ConvertFrom','datenum')-seconds(window_length_seconds))])
%     datetick('x',13,'keeplimits')
%     title(strcat([str_region{i},'; ',datestr(NaN_data.Time_datetime(1),'yyyy-mm-dd')]))
%     for j = 1:length(parameters_of_interest)
%         plot(n11,0,'*','color',color_parameters1(parameters_of_interest(j)-1,:))
%     end
%     legend(variables{parameters_of_interest})%2:5})
%     for j = parameters_of_interest%2:5
%         for k = 1:size(P_Fregion{i,j},2)-5
%             if isempty(locs{i,j,k}) == 0
%                 plot(Time1(k),locs{i,j,k}*1000,'*','color',color_parameters1(j-1,:),'HandleVisibility','off')
%             else ;
%             end
%         end
%     end
