...
set(gcf,'Units',unifig,'Position',posfig,'DefaultAxesFontSize',fontsize_axes,...
  ...
  'DefaultTextHorizontalAlignment','center','InvertHardCopy','off')
...
%       end
      ...
      hold on
      plot(xx,yy,'-','linewidth',1,'color',linecolor) % plot error bar first
      
      plot(xdata,ydata,lsp{j},'linewidth',1.5,'color',linecolor)
...
%       end
    ...
    if m>1
    hl=axes_legend(hf,ha{i},legends,co,fontsize_text-2,[5,2]);
    end
    % xlim
    Xlim=[t_st t_ed];
...
%       'fontsize',10,'fontweight','normal')
  ...
  elseif plottype==2
    Ylabel=axes_axis_label(para.drawopt.ylabel,para.drawopt.yunit,1);
  end
  % set axis
  set(gca,                              ...
        'xtick',        xtick,          ...
...
if drawopt.save==1
  ...
  for nn=1:length(opt_print)
    ...
    switch figformat
      case 'tiff' 
        print('-dtiff','-r300',[fp_res fn])
      case 'png'
        print('-dpng','-r500',[fp_res fn]);
      case 'eps'
        print('-depsc','-r300',[fp_res fn]);
    end
  end
end
